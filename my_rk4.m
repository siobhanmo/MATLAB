...
% y is 1 × (n + 1) array of class double that represents the values of the function y at the times t (in the same order) as approximated by the explicit Euler method.
    ...
    for i=1: length(t)-1
        k1 = f(t(i),y(i));
        k2 = f(t(i)+(t(1+i)-t(i))/2,y(i)+k1*(t(1+i)-t(i))/2);
        k3 = f(t(i)+(t(1+i)-t(i))/2,y(i)+k2*(t(1+i)-t(i))/2);
        k4 = f(t(i)+(t(1+i)-t(i)),y(i)+k3*(t(1+i)-t(i)));
        y(i+1) = y(i)+ (1/6)*(k1+2*k2+2*k3+k4)*(t(1+i)-t(i));
    end
